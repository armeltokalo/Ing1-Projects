...
def is_log_line_processed(cursor, log_hash):
    ...
    """
    cursor.execute("SELECT COUNT(*) FROM ecp_logs WHERE log_hash = %s", (log_hash,))
...
def process_log_file(input_file_path, folder_name, cursor, connection):
    try:
        ...
        print(f"Lignes traitées et inserées: {processed_lines}, Lignes ignorées : {skipped_lines}")
    ...
    while True:
        try:
            ...
            with connection.cursor() as cursor:
                directory_path = "C:/Users/Admin/Desktop/Projet Info/logs/httpsproxy/Ecp"
